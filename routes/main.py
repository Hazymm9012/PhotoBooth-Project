import string
import uuid
from flask import Flask, abort, render_template, request, redirect, url_for, session, jsonify, send_file, flash, Blueprint, current_app
...
from models import Photo, PhotoStatus, Payment
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if ok:
            ...
            return redirect(url_for("main.admin_download"))
...
def get_secure_image_url(filename, add_expiration=True, download=False):
    ...
    secure_link = url_for('main.view_secure_image', token=token, download=download, _external=True)
    return secure_link
...
def payment():
    ...
    #save_image()
    return redirect(url_for('main.payment_summary'))
...
def create_payment_request():
    ...
    redirect_url = url_for("main.redirect_user", _external=True)                       # Use the Hitpay redirect URL from the config
...
def success():
    ...
    if not stored_payment:
        ...
        return redirect(url_for('main.fail', payment_request_id=payment_id, status='failed'))
